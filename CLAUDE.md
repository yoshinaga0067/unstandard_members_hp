...
- `/start` — `develop` に切り替えて最新を pull します。新規プロジェクトでは「何を作りますか？」から会話を始め、合意できてから初めてセットアップを実行します（事前にテーマやデータベースを質問攻めにしません）。既存プロジェクトではプロジェクトの状態を日本語で要約します。セッション開始時に必ず使います。
